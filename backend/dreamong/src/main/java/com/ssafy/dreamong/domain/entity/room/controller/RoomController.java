...
public class RoomController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<RoomDetailsResponse>>> getAllRooms() {
        try {
            List<RoomDetailsResponse> allRooms = roomService.getAllRooms();
            if (allRooms.isEmpty()) {
                return new ResponseEntity<>(ApiResponse.success(), HttpStatus.NO_CONTENT);
            }
            return new ResponseEntity<>(ApiResponse.success(allRooms), HttpStatus.OK);
        } catch (Exception e) {
            log.error("방 전체 조회 중 오류:{}", e.getMessage());
    ...
    @GetMapping("/{roomId}")
    public ResponseEntity<ApiResponse<RoomDetailsResponse>> getRoomById(@PathVariable Integer roomId) {
        try {
            RoomDetailsResponse findRoom = roomService.getRoomDetailsById(roomId);
            return ResponseEntity.ok(ApiResponse.success(findRoom));
        } catch (RoomNotFoundException e) {
            log.info("방 상세 번호 없음:{}", e.getMessage());
            return new ResponseEntity<>(ApiResponse.error(), HttpStatus.NOT_FOUND);
        } catch (Exception e) {
            log.info("방 상세 조회 중 오류:{}", e.getMessage());
            return new ResponseEntity<>(ApiResponse.error(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
